...
function Modulecard(props) {
 ...
 /*  const handleCircleClick = async (details) => {
    try {
      ...
      const userResponse = await axios.get(`http://localhost:5030/get-user/${entityID}`);
      const user = userResponse.data;
  
      // Check if OrgID from user details matches OrgID in module details
      if (user && user[0].OrgID === details.OrgID) {
        const teamsResponse = await axios.get(`http://localhost:5030/get-user-teams/${entityID}`);
          ...
          if (selectedTeam) {
            ...
            if (selectedTeamDetails) {
              ...
              const updateAssignedModulesResponse = await axios.put(
                `http://localhost:5030/update-assigned-modules/${selectedTeamDetails._id}`,
              ...
              const appendTeamsResponse = await axios.put(
                `http://localhost:5030/append-teams/${details._id}`,
