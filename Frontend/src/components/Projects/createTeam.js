...
function CreateTeam() {
  ...
  useEffect(() => {
    ...
    const fetchTeamMembers = async () => {
      try {
        const res = await axios.get(`http://10.8.0.13:5030/get-users-by-org-for-team/${entityID}`);
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(`http://10.8.0.13:5030/organizationbyid/${entityID}`);
  ...
  const handleAddTeam = async () => {
    try {
      ...
      // Step 2: Create team
      const createTeamResponse = await axios.post('http://10.8.0.13:5030/create-team', {
        TeamName: teamName, 
