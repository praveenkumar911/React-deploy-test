...
useEffect(() => {
  const fetchData = async () => {
    try {
      // Fetch project data
      const projectResponse = await axios.get(`http://localhost:5030/get-project/${projectID}`);
      const projectData = projectResponse.data;

      // Set project data
      setProjectData(projectData);
      setGitProjectName(projectData.Gitprojectname);
      setProjectOwner(projectData.projectOwner);

      // Fetch OrgID based on projectowner
      const orgIDResponse = await axios.get(`http://localhost:5030/get-orgID/${projectData.projectOwner}`);
      setOrgID(orgIDResponse.data.orgID); // Assuming the OrgID is returned from the API
      console.log(OrgID);
    } catch (error) {
      console.log(error);
    }
...
<Grid container xs={12} style={{ display: 'flex', alignItems: 'center', marginLeft: '0.2vw' }}>
  {/* First Row */}
...
{/* Fifth Row - Others */}
{/* <Grid item xs={12} style={{ display: 'flex', alignItems: 'center', marginLeft: '0.2vw' }}>
  <input type="file" style={{ display: 'none' }} ref={fileInputRef4} onChange={handleFileChange4} />
  <div style={{ display: 'flex', alignItems: 'center',cursor:"pointer"  }} onClick={() => handleClick4('Others')}>
    <img src={material} alt="Others" style={{ width: '24px', height: '24px', marginRight: '8px' }} />
    <span style={{ fontSize: '14px', fontWeight: '500', marginRight: '20px' }}>Others</span>
  </div>
  {fileName5 && <span>{fileName5}</span>}
</Grid> */}

  {/* Third Row - Add similar structures for API Document, DB Document, and Others */}
</Grid>
